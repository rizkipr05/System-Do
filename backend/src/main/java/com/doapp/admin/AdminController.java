package com.doapp.admin;

import com.doapp.address.Address;
import com.doapp.address.AddressRepository;
import com.doapp.address.dto.AddressDto;
import com.doapp.admin.dto.AdminCreateOrderRequest;
import com.doapp.admin.dto.AdminCustomerDto;
import com.doapp.admin.dto.AdminCustomerRequest;
import com.doapp.admin.dto.AdminOrderDto;
import com.doapp.admin.dto.AdminOrderItemDto;
import com.doapp.admin.dto.AdminProductDto;
import com.doapp.admin.dto.AdminReportSummaryDto;
import com.doapp.admin.dto.AdminUpdateStatusRequest;
import com.doapp.admin.dto.AssignDriverRequest;
import com.doapp.admin.dto.AdminReportItemDto;
...
public class AdminController {
  ...
  public AdminController(AuthHelper authHelper,
  ...
  public List<AdminCustomerDto> customers(@RequestHeader("Authorization") String authHeader) {
    authHelper.requireAdmin(authHeader);
  ...
  public AdminCustomerDto createCustomer(@RequestHeader("Authorization") String authHeader,
                                         @RequestBody AdminCustomerRequest req) {
    authHelper.requireAdmin(authHeader);
  ...
  public AdminCustomerDto updateCustomer(@RequestHeader("Authorization") String authHeader,
  ...
  public List<AdminOrderDto> customerOrders(@RequestHeader("Authorization") String authHeader,
                                            @PathVariable Long userId) {
    authHelper.requireAdmin(authHeader);
    Customer c = customerRepo.findByUserId(userId)
        .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Customer tidak ditemukan"));
    return orderRepo.findByCustomerIdOrderByCreatedAtDesc(c.getId())
        .stream().map(AdminController::toAdminOrderDto).toList();
  }
  ...
  public List<AddressDto> customerAddresses(@RequestHeader("Authorization") String authHeader,
    ...
    return addressRepo.findByCustomerIdOrderByIsDefaultDescIdDesc(c.getId())
        .stream().map(AdminController::toAddressDto).toList();
  }
  ...
  @GetMapping("/products")
  public List<AdminProductDto> products(@RequestHeader("Authorization") String authHeader) {
    authHelper.requireAdmin(authHeader);
  ...
  public AdminProductDto createProduct(@RequestHeader("Authorization") String authHeader,
                                       @RequestBody AdminProductDto req) {
    authHelper.requireAdmin(authHeader);
    ...
    productRepo.save(p);
    return new AdminProductDto(p.getId(), p.getName(), p.getSku(), p.getUnit(), p.getPrice(), p.getStock(), p.isActive());
  }

  @PutMapping("/products/{id}")
  public AdminProductDto updateProduct(@RequestHeader("Authorization") String authHeader,
                                       @PathVariable Long id,
                                       @RequestBody AdminProductDto req) {
    authHelper.requireAdmin(authHeader);
  ...
  @GetMapping("/orders")
  public List<AdminOrderDto> orders(@RequestHeader("Authorization") String authHeader) {
    authHelper.requireAdmin(authHeader);
    return orderRepo.findAllByOrderByCreatedAtDesc().stream()
        .map(AdminController::toAdminOrderDto).toList();
  }

  @PostMapping("/orders/manual")
  public AdminOrderDto createManualOrder(@RequestHeader("Authorization") String authHeader,
                                         @RequestBody AdminCreateOrderRequest req) {
    authHelper.requireAdmin(authHeader);
    if (req.customerId() == null) throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Customer wajib dipilih");
    ...
    Customer c = customerRepo.findById(req.customerId())
        .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Customer tidak ditemukan"));
    ...
    for (AdminCreateOrderRequest.Item item : req.items()) {
    ...
    return toAdminOrderDto(orderRepo.save(o));
  }

  @PostMapping("/orders/{id}/status")
  public AdminOrderDto updateStatus(@RequestHeader("Authorization") String authHeader,
                                    @PathVariable Long id,
                                    @RequestBody AdminUpdateStatusRequest req) {
    authHelper.requireAdmin(authHeader);
  ...
  public AdminOrderDto assignDriver(@RequestHeader("Authorization") String authHeader,
                                    @PathVariable Long id,
                                    @RequestBody AssignDriverRequest req) {
    authHelper.requireAdmin(authHeader);
    if (req.driverId() == null)
      throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Driver wajib dipilih");
  ...
  public AdminOrderDto reschedule(@RequestHeader("Authorization") String authHeader,
                                  @PathVariable Long id) {
    authHelper.requireAdmin(authHeader);
    DeliveryOrder o = orderRepo.findById(id)
        .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Order tidak ditemukan"));
  ...
  public AdminReportSummaryDto reportSummary(@RequestHeader("Authorization") String authHeader) {
    authHelper.requireAdmin(authHeader);
    ...
    for (DeliveryOrder o : orders) {
      String customerName = o.getCustomer().getUser().getName();
    ...
    List<AdminReportItemDto> topProducts = productTotals.entrySet().stream()
        .sorted(Map.Entry.comparingByValue(Comparator.reverseOrder()))
        .limit(5)
        .map(e -> new AdminReportItemDto(e.getKey(), e.getValue()))
        .toList();

    List<AdminReportItemDto> topCustomers = customerTotals.entrySet().stream()
        .sorted(Map.Entry.comparingByValue(Comparator.reverseOrder()))
        .limit(5)
        .map(e -> new AdminReportItemDto(e.getKey(), e.getValue()))
        .toList();
  ...
  private static AdminOrderDto toAdminOrderDto(DeliveryOrder o) {
    List<AdminOrderItemDto> items = o.getItems().stream()
        .map(i -> new AdminOrderItemDto(i.getProduct().getId(), i.getProduct().getName(), i.getProduct().getPrice(), i.getQuantity()))
        .toList();
    String driverName = o.getDriver() == null ? null : o.getDriver().getName();
